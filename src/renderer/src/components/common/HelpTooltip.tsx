...
export function HelpTooltip({ content, className, iconClassName }: HelpTooltipProps) {
  ...
  return (
    ...
    >
      <HelpCircle
        className={cn(
          'h-3 w-3 cursor-help text-muted-foreground/30 transition-colors hover:text-muted-foreground/60',
      ...
      <AnimatePresence>
        {isVisible && (
          ...
          >
            {/* Arrow */}
            <div className="absolute -bottom-[5px] left-1/2 h-2.5 w-2.5 -translate-x-1/2 rotate-45 border-b border-r border-border/60 bg-popover" />
