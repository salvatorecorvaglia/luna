...
const CLI_DOCS: Record<string, CliCommandDoc> = {
  tar: {
    ...
    category: 'files',
    examples: [
      { description: 'Create gzipped archive', command: 'tar -czvf archive.tar.gz /path/to/directory' },
      { description: 'Extract gzipped archive to current dir', command: 'tar -xzvf archive.tar.gz' },
      { description: 'List contents of a tar archive', command: 'tar -tzvf archive.tar.gz' },
      { description: 'Extract to specific destination folder', command: 'tar -xzvf archive.tar.gz -C /target/dir' },
    ],
  },
  find: {
    name: 'find',
    ...
    category: 'files',
    examples: [
      { description: 'Find files by extension recursively', command: "find . -type f -name '*.log'" },
  ...
  grep: {
    ...
    category: 'files',
    examples: [
      { description: 'Case-insensitive search recursively', command: "grep -rnI 'error' /var/log/" },
      ...
      { description: 'Invert match (show lines NOT matching)', command: "grep -v 'DEBUG' server.log" },
    ],
  },
  rsync: {
    name: 'rsync',
    ...
    category: 'network',
    examples: [
      { description: 'Archive sync with progress bar', command: 'rsync -avP /local/dir user@remote:/remote/dir' },
      { description: 'Sync and delete extraneous files at dest', command: 'rsync -av --delete /src/ /dest/' },
  ...
  docker: {
    ...
    examples: [
      ...
      { description: 'Tail live logs of container', command: 'docker logs -f --tail 100 <container_id>' },
      { description: 'Execute interactive bash in container', command: 'docker exec -it <container_id> /bin/bash' },
      { description: 'Clean up unused containers and volumes', command: 'docker system prune -a --volumes' },
    ],
  },
  kubectl: {
    name: 'kubectl',
    ...
    examples: [
      ...
      { description: 'Stream container pod logs', command: 'kubectl logs -f <pod_name> -n <namespace>' },
      { description: 'Describe pod events and status', command: 'kubectl describe pod <pod_name>' },
      { description: 'Port forward local port to pod', command: 'kubectl port-forward pod/<pod_name> 8080:80' },
    ],
  },
  systemctl: {
    name: 'systemctl',
  ...
  chmod: {
    ...
    examples: [
      ...
      { description: 'Set read/write/execute for owner, read/execute for others', command: 'chmod 755 app.sh' },
  ...
  ffmpeg: {
    ...
    category: 'devops',
    examples: [
      { description: 'Convert MP4 video to WebP animation', command: 'ffmpeg -i input.mp4 -vf "fps=10,scale=800:-1" out.webp' },
      { description: 'Extract audio track as MP3', command: 'ffmpeg -i input.mp4 -vn -acodec libmp3lame out.mp3' },
      { description: 'Compress video with H.264 CRF 23', command: 'ffmpeg -i input.mp4 -vcodec libx264 -crf 23 out.mp4' },
    ],
  },
};
  ...
  searchDocs(query?: string): CliCommandDoc[] {
    ...
    return Object.values(CLI_DOCS).filter(
      (doc) =>
        ...
        doc.examples.some((ex) => ex.description.toLowerCase().includes(q) || ex.command.toLowerCase().includes(q)),
    );
  }
